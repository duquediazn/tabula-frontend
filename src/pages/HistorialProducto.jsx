...
import { getHistorialProducto } from "../api/stock";
...
export default function HistorialProducto() {
  ...
  useEffect(() => {
    const fetchGrafico = async () => {
      try {
        const data = await getHistorialProducto(id, accessToken, 100, 0);
  ...
  useEffect(() => {
    document.title = "Historial Producto";
  }, []);

  // Tabla (limit = 10 con offset)
  useEffect(() => {
    const fetchTabla = async () => {
      try {
        setIsLoading(true);
        const data = await getHistorialProducto(id, accessToken, limit, offset);
